...
gameContainer.addEventListener('touchmove', function(e){ e.preventDefault(); moveBasket(e);}, {passive:false});
...
function moveHearts(){
    hearts.forEach((heart, idx)=>{
        ...
        if(!(heartRect.right<basketRect.left||heartRect.left>basketRect.right||heartRect.bottom<basketRect.top||heartRect.top>basketRect.bottom)){
            score++;
...
// Game loop
function gameLoop(){ if(!isPaused) moveHearts(); }
...
function startGame(){
    ...
    heartGenInterval = setInterval(()=>{ if(!isPaused&&score<maxScore) createHeart(); },800);
}
...
okGameOverBtn.addEventListener('click', ()=>{ gameOverOverlay.style.display='none'; });
